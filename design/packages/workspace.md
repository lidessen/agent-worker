...
- `worktree.ts` — thin wrapper over `git worktree add/remove/list`; branch naming `workspaceKey:agentName:attemptId`; path under `workspaceSandboxDir/worktrees/`.
...
**`loop/`** — dispatch primitives used by the orchestrator. `priority-queue.ts` implements the three-lane `InstructionQueue` with starvation protection and cooperative preemption. `prompt.tsx` defines composable `PromptSection` building blocks. `lead-hooks.ts` wires workspace-lead task management (chronicle append, handoff creation on specific events).
...
Worktrees are created on-demand via the `worktree_create` tool inside a running attempt, not preallocated. Provisioning calls `git worktree add` with a branch name scoped to `workspaceKey:agentName:attemptId`. On attempt terminal status (`completed | failed | cancelled | handed_off`), worktrees for that attempt are removed; **branches are left as audit trail.** Per-run tool rebinding in `WorkspaceMcpHub` attaches the `activeAttemptId`, which is why `worktree_*` tools appear iff an attempt is active.
...
- **Promotion:** `background` items promote to `normal` after a wait threshold or N preemptions.
- **Cooperative preemption:** `shouldYield()` signals the agent to save progress; re-queued items resume via `preemptionCount` + `progressState`.
