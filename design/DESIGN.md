...
The daemon starts, discovers runtimes, and processes tasks through the coordination harness. Core loop verified (task create → dispatch → Codex agent runs in worktree → handoff completes). Three blockers prevent daily use; coordination is the next differentiating capability.

**External landscape.** Multica (27K stars, Go daemon + Next.js, 10+ CLI backends) treats agents as issue assignees on a shared board — thin daemon, thick server, no agent-to-agent coordination. Claude Code Agent View (v2.1.139) treats sessions as process-level primitives — parallel independent workers with zero cross-session state sharing. Both validate the market direction (agents as teammates) and agent-worker's differentiation (Harness as shared context, Wake/Handoff structured state transfer, OSS anchoring).

**Priority.** (1) Three daily-use blockers in `HANDOFF.md` → (2) coordination end-to-end (two agents in one harness, channel routing, Wake handoff) → (3) monitor producing C1–C4 numbers. The substrate cut (extracting coordination into `internals/harness-coordination/`) is deferred — it's structural refinement without behavioral change, and the current in-harness coordination works.
