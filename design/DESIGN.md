...
| Doc | Scope |
| --- | --- |
| [packages/agent-worker.md](packages/agent-worker.md) | Daemon, CLI, HTTP surface, harness registries, workspace orchestrator |
...
| [packages/workspace.md](packages/workspace.md) | `WorkspaceHarness` kernel — Signals, WorkspaceEvents, projections, capability boundary, Wake / Handoff records, MCP tools |
...
HTTP / CLI / Web ──┼─► HarnessRegistry / lifecycle                    │
                   ...
                   │       │   ├─ task projections (per harness)      │
                   │       │   ├─ execution records (Wake / Handoff)  │
...
_Does:_ `Workspace` definition; raw channels/inbox as intake and audit evidence; semantic `WorkspaceEvent` stream; resource pointers; rebuildable Track and task projections; runtime-boundary records (`Wake`, `Handoff`); protected invocation records; context packet builders; capability validation contracts; MCP/tool surfaces that submit typed invocations; Wake-scoped git worktree provisioning. (See [decisions/005](decisions/005-session-orchestration-model.md): Task moves out of the kernel into a harness-layer projection; `Artifact` is flagged for merge with `Resource`.)
...
WorkspaceOrchestrator.tick
      ...
      ├─ harness runs produceExtension hook → committed Handoff (core + ext.)
...
Core types: `BusEvent` (shared telemetry), `LoopEvent` (loop telemetry), `RuntimeBinding` / `ContextPacket` / `ToolCapabilitySet` / `RunPolicy` / `RuntimeTrace` / `ExecutionResult` / `HandoffDraft` (runtime boundary), `Signal` / `WorkspaceEvent` / `CapabilityInvocation` / `Track` / `Wake` / `Handoff` (core + per-harness extension) / `Resource` / `Worktree` (workspace harness), `TaskProjection` (harness-layer projection over events; not a kernel record), optional `AgentState` / `Turn` / `AssembledPrompt` only inside a personal/simple harness migration.
...
**Wake-scoped worktrees, not per-agent globals.** Worktrees are created on-demand by a tool call within a running Wake and torn down at terminal status; branches survive. _Rejected:_ pre-allocate one worktree per agent at workspace startup. _Why:_ clean task isolation, no cross-task branch pollution, no git state in workspace schema, deterministic cleanup driven by Wake lifecycle.

**WorkspaceEvent over raw transcript as the workspace context subject.** Workspace mode uses semantic events and projections for long-term coordination, while L0 Wakes keep raw conversations local to execution and audit evidence. _Rejected:_ keep channels, timeline, chronicle, and prompt sections as competing long-term state surfaces. _Why:_ multi-day and multi-agent work needs high-density facts, bounded context packets, and replayable projections rather than a growing chat transcript.
