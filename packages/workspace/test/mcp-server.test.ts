...
describe("WorkspaceMcpHub + WorkspaceClient", () => {
  ...
  test("server starts and reports health", async () => {
    ...
    const data = (await res.json()) as { status: string; workspace: string };
  ...
  test("inbox tools work via MCP", async () => {
    ...
    // so the workspace routes the message to bob's inbox.
    await workspace.registerAgent("bob", ["general"]);
  ...
  test("two agents collaborate via MCP", async () => {
    const alice = new WorkspaceClient({
      agentName: "alice",
      workspaceUrl: server.url!,
    });
    const bob = new WorkspaceClient({
      agentName: "bob",
      workspaceUrl: server.url!,
    });
...
describe("WorkspaceMcpHub debug tools", () => {
  let workspace: Workspace;
  let server: WorkspaceMcpHub;
  ...
  test("events tool filters by agent", async () => {
    await workspace.eventLog.log("alice", "system", "Alice event");
    await workspace.eventLog.log("bob", "system", "Bob event");
  ...
  test("workspace_info shows workspace configuration", async () => {
