/**
 * Workspace MCP Hub — exposes workspace tools via HTTP MCP protocol.
 *
...
export interface WorkspaceMcpHubOptions {
...
export class WorkspaceMcpHub {
  ...
  async start(opts?: WorkspaceMcpHubOptions): Promise<void> {
    ...
    this.httpServer = createServer(async (req, res) => {
      ...
      try {
        ...
        console.error(`[workspace-mcp:${name}] error:`, err);
  ...
  private async createAgentServer(agentName: string): Promise<McpServer> {
    if (!this.workspace.hasAgent(agentName)) {
      await this.workspace.registerAgent(agentName);
    }

    const server = new McpServer({
      name: `workspace-${this.workspace.name}-${agentName}`,
      version: "0.0.1",
    });
  ...
  private registerDebugTools(server: McpServer): void {
    ...
    // ── events: filtered workspace events ───────────────────────────────
    server.tool(
      "events",
      "Query workspace events across all agents, with optional filtering by agent name and/or event kind. Returns most recent events first.",
      {
    ...
    server.tool(
      "queue",
      "Show all pending instructions in the workspace queue, grouped by priority.",
      async () => {
    ...
    // ── workspace_info: workspace configuration overview ────────────────
    server.tool(
      "workspace_info",
      "Show workspace configuration: name, channels, agent count, queue settings, and storage paths.",
      async () => {
