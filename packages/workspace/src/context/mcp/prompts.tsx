...
import type { PromptSectionNode } from "../../loop/prompt-ui.tsx";
    ...
    <section title="Workspace">
      ...
      {isLead && (
        <>
          ...
          <item>You are responsible for responding to user messages, even if they don&apos;t @ you directly.</item>
          <item>Unmentioned messages from users (e.g. telegram) are routed to you at normal priority.</item>
      ...
      <item>
        `channel_send` posts to channels. Plain text output is your private thinking -- only you
        see it.
      </item>
      <item>`@name` in messages notifies that teammate.</item>
      <item>
        Messages over 1200 chars: use `resource_create` first, then send a summary with the
        resource ID.
      </item>
      ...
      <field label="Shared workspace" value={`\`${ctx.workspaceSandboxDir ?? "(not available)"}\``} />
      <br />
      <line>Channels</line>
      {channels.length > 0 ? channels.map((channel) => <item key={channel}>{`#${channel}`}</item>) : <item>(none)</item>}
      {teammates.length > 0 && (
        <>
          <br />
          <line>Teammates</line>
...
export const conversationSection: PromptSection = async (ctx) => {
  ...
  const channelSections = await Promise.all(channels.map(async (ch) => {
