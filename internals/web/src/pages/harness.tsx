...
import type { HarnessInfo, DocInfo, TaskSummary, TaskDetail } from "../api/types.ts";
import * as styles from "./harness.style.ts";

export const HarnessPage: RuntimeComponent<Record<string, never>> = (_props, ctx) => {
  const wsKey = computed(route, (r) => (r.page === "harness" ? r.params.key : ""));
  ...
  async function loadHarness(key: string, force = false) {
    if (!key) return;
    if (key === currentKey && !force) return;
    currentKey = key;
    error.value = null;
    // Reset list signals so a fast navigation doesn't leave stale rows
    // from the previous harness visible while the new fetch is in flight.
    tasks.value = [];
    // Tear down any event stream bound to the previous harness.
    ...
    try {
      const [ws, ch, docList, taskList] = await Promise.all([
        ...
        c.listChannels(key),
        c.listDocs(key),
        c.listHarnessTasks(key).catch((err) => {
      ...
      // Subscribe to the harness event stream and refresh tasks whenever
      // a harness.task_changed event lands. Debounce so a burst of
      // changes (dispatch + status transition + handoff) only triggers
      // one refetch.
      subscribeForTaskUpdates(key);
    } catch (err) {
      console.error(`Failed to load harness ${key}:`, err);
  ...
  function subscribeForTaskUpdates(key: string) {
    ...
    const scheduleRefresh = (changedTaskId?: string) => {
      ...
      refreshTimer = setTimeout(async () => {
        ...
        try {
          const fresh = await liveClient.listHarnessTasks(key);
    ...
    void (async () => {
      try {
        for await (const event of c.streamHarnessEvents(key, { signal: controller.signal })) {
          if (controller.signal.aborted) return;
          const ev = event as { type?: string; taskId?: string };
          if (ev.type === "harness.task_changed") {
        ...
        if (name !== "AbortError") {
          console.warn(`Harness event stream for ${key} ended:`, err);
        }
      }
    })();
  }

  loadHarness(wsKey.value);
  ...
  const unsubClient = client.subscribe((c) => {
    if (c && wsKey.value && !harness.value) {
      loadHarness(wsKey.value, true);
    }
  });
  ...
  async function loadTaskDetail(taskId: string) {
    ...
    if (!c || !key) return;
    try {
      const detail = await c.getHarnessTask(key, taskId);
      // Guard against stale arrivals after navigation away.
      if (currentKey !== key) return;
  ...
  const wsNameDisplay = computed([harness, wsKey], (ws, key) => ws?.name ?? key);
  ...
  const agentsSection = computed(harness, (ws) => {
  ...
  return (
    <div class={styles.page} data-page="harness">
