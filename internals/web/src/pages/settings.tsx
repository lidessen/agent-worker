...
const healthRows = computed(healthInfo, (info) => {
  ...
  return [
    ...
    <div class={styles.infoRow}>
      <span class={styles.infoLabel}>Harnesses</span>
      <span class={styles.infoValue}>{String(info.harnesses)}</span>
    </div>,
  ];
});
...
export const SettingsPage: RuntimeComponent<Record<string, never>> = (_props, ctx) => {
  ...
  async function handleTest() {
    ...
    try {
      ...
      testResult.value = {
        ok: true,
        message: `Connected — ${info.agents} agents, ${info.harnesses} harnesses`,
      };
    } catch (err: unknown) {
