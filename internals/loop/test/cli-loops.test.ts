...
function mockCliConfig(
  format: string,
  scenario: string,
  mapEvent: CliLoopConfig["mapEvent"],
  extractResult: CliLoopConfig["extractResult"],
): CliLoopConfig {
  return {
    command: "bun",
  ...
  switch (type) {
    case "message":
      if ((event.role as string) === "assistant")
        return { type: "text" as const, text: (event.content as string) ?? "" };
      return null;
    case "function_call":
    case "tool_call":
      return {
...
describe("CodexLoop (mock)", () => {
  ...
  test("error scenario", async () => {
    const run = runCliLoop(mockCliConfig("codex", "error", mapCodexEvent, extractCodexResult), {});
...
describe("Cross-runtime contract", () => {
  test("all runtimes produce LoopRun with result promise", async () => {
    const configs = [
      {
        name: "claude",
        config: mockCliConfig("claude", "success", mapClaudeEvent, extractClaudeResult),
      },
      {
        name: "codex",
