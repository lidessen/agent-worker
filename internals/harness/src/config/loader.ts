...
import type { HarnessConfig, ChannelAdapter } from "../types.ts";
 ...
 * Supports dotted paths like `${{ harness.tag }}`.
 */
 ...
 * Centralise the whitelist here so a typo at harness creation
...
/** Parse a YAML string into a HarnessDef. */
export function parseHarnessDef(content: string): HarnessDef {
  const raw = parseYaml(content);

  if (!raw || typeof raw !== "object") {
    throw new Error("Invalid harness definition: expected an object");
  }
...
/**
 * Load a harness definition from a YAML file or string, run setup steps,
 * and interpolate templates. Returns the resolved harness.
 */
export async function loadHarnessDef(
  ...
  if (pathOrContent.includes("\n") || pathOrContent.trimStart().startsWith("agents:")) {
    ...
    try {
      ...
      throw new Error(`Harness definition not found: ${filePath}`);
    }
  ...
  const def = parseHarnessDef(content);
  const rawHarnessMcp =
    (def as HarnessDef & { mcp?: Record<string, unknown>; mcpServers?: Record<string, unknown> })
      .mcp ??
    (
      def as HarnessDef & {
    ...
    (def as HarnessDef & { mcpServers?: Record<string, unknown> }).mcpServers;
  const harnessMcpServers =
    rawHarnessMcp && typeof rawHarnessMcp === "object"
      ? Object.fromEntries(
          Object.entries(rawHarnessMcp).map(([serverName, serverDef]) => [
            serverName,
  ...
  if (!def.name) {
    throw new Error(
      "Harness name is required: set 'name' in YAML, use a named file, or pass opts.name",
    );
  }
  ...
  for (const [name, agentDef] of Object.entries(def.agents ?? {})) {
    ...
    // Merge harness-level env + agent-level env (agent wins)
    ...
    // Role: explicit override wins; otherwise lead iff harness.lead matches.
    ...
    const agentMcpServers =
      rawMcp && typeof rawMcp === "object"
        ...
        : undefined;
    const mcpServers =
      harnessMcpServers || agentMcpServers
        ? {
            ...harnessMcpServers,
            ...agentMcpServers,
          }
    ...
    // Phase 3 control-boundary policy: harness provides the
  ...
  const baseVars: Record<string, string> = {
    ...opts.vars,
    "harness.name": def.name,
  };
  if (opts.tag) {
    baseVars["harness.tag"] = opts.tag;
  }
  ...
  return {
    def: def as HarnessDef & { name: string },
    agents,
...
export interface ToHarnessConfigOptions extends LoadOptions {
...
/**
 * Convert a resolved harness definition into a HarnessConfig
 * suitable for createHarness().
 */
export function toHarnessConfig(
  resolved: ResolvedHarness,
  opts: ToHarnessConfigOptions = {},
): HarnessConfig {
  const { def } = resolved;
