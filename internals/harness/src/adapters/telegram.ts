/**
 * Telegram adapter for harness channel bridge.
 *
...
export class TelegramAdapter implements ChannelAdapter {
  ...
  private async sendStatus(chatId: number): Promise<void> {
    const lines = [
      ...
      `Harness: ${this.bridge ? "connected" : "not connected"}`,
  ...
  private handleMessage(msg: TelegramMessage): void {
    ...
    if (!this.bridge) {
      this.api("sendMessage", {
        chat_id: msg.chat.id,
        text: "No harness connected. Start a harness with this Telegram connection first.",
      }).catch(() => {});
      return;
    }
