...
import { runProduceExtension, DEFAULT_HARNESS_TYPE_ID } from "../../type/index.ts";

/**
 * Task / Wake / Handoff MCP tools. These expose the harness kernel state
...
export interface TaskToolsDeps {
   ...
   * the harness chronicle under the "task" category, so external
  ...
  /**
   * `HarnessType` registry consulted by `handoff_create` to run the
   * type's `produceExtension` hook before persisting. Defaults to a
   * fresh registry holding only the no-op default type — fine for tests
   * and for any caller that hasn't plugged in a concrete type yet.
   */
  harnessTypeRegistry?: HarnessTypeRegistry;
  /**
   * `HarnessType` id this Harness is plugged into. Defaults to
   * `DEFAULT_HARNESS_TYPE_ID`. Recorded on every Handoff this tool
   * writes so the read site knows which type's `consumeExtension` to
   * run later.
   */
...
export function createTaskTools(
  ...
  const harnessTypeId = deps.harnessTypeId ?? DEFAULT_HARNESS_TYPE_ID;
  ...
  return {
    ...
    async task_update(args): Promise<string> {
      ...
      const patch: Parameters<HarnessStateStore["updateTask"]>[1] = {};
    ...
    async wake_update(args): Promise<string> {
      ...
      const patch: Parameters<HarnessStateStore["updateWake"]>[1] = {};
      if (args.status !== undefined) patch.status = args.status;
    ...
    async handoff_create(args): Promise<string> {
      ...
      try {
        ...
        // persisting. Decision 005 + 006: produce-throw is logged and the
        // core still writes (missing extension is recoverable). Slice 1
        // ships only the default no-op type, so this is a no-op today;
        // concrete types plug in from slice 3 onward.
        const extensionEntry = deps.harnessTypeRegistry
          ? await runProduceExtension(deps.harnessTypeRegistry, harnessTypeId, {
              wake: (await store.getWake(args.closingWakeId)) ?? {
                id: args.closingWakeId,
              } as Wake,
              // Slice 1 hands an empty events array; the work-log /
              // event-window slice will populate this.
              events: [],
              workLog: undefined,
              draft: {
    ...
    async task_dispatch(args): Promise<string> {
      if (!deps.instructionQueue) {
        return "Error: task_dispatch unavailable — this harness has no instruction queue.";
      }
...
export const TASK_TOOL_DEFS = {
  task_create: {
    description:
      "Create a new task in the harness ledger. Defaults to status 'draft' — " +
  ...
  task_list: {
    description: "List tasks in the harness ledger, optionally filtered by status or owner.",
    parameters: {
      status: {
        type: "string",
  ...
  task_dispatch: {
    description:
      ...
      "enqueues an instruction on the worker's queue. Only available when the harness has " +
      "an instruction queue (i.e. on live Harness runtimes, not bare test harnesses).",
    parameters: {
