 ...
 * the `StreamableHTTPServerTransport` on the HarnessMcpHub, but
 * codex's app-server deadlocks mid-tool-call on that transport:
 * the tool runs, the harness side-effect happens, but codex
 ...
 *
 * The harness MCP server uses the stdio path (prefers bun, falls
 * back to node+tsx). `httpUrl` is accepted for backward compatibility
 * but ignored.
 */
export async function createHarnessMcpConfig(
  agentName: string,
  runtime: string,
  opts: {
  ...
  const configPath = `/tmp/harness-mcp-${agentName}-${Date.now()}.json`;
