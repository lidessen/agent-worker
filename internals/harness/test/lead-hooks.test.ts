...
describe("buildLeadHooks.onCheckpoint", () => {
  test("first run emits no delta — nothing to compare against", async () => {
    ...
    await store.createTask({ harnessId: "w1", title: "a", goal: "g" });
  ...
  test("reports new tasks since the last snapshot", async () => {
    const store = new InMemoryHarnessStateStore();
    await store.createTask({ harnessId: "w1", title: "first", goal: "g" });
    const hooks = buildLeadHooks(store);
    ...
    await store.createTask({ harnessId: "w1", title: "second", goal: "g" });
  ...
  test("reports removed tasks when a task leaves the tracked set", async () => {
    const store = new InMemoryHarnessStateStore();
    const task = await store.createTask({ harnessId: "w1", title: "bye", goal: "g" });
    const hooks = buildLeadHooks(store);
  ...
  test("emits noop when nothing changes between runs", async () => {
    const store = new InMemoryHarnessStateStore();
    await store.createTask({ harnessId: "w1", title: "still here", goal: "g" });
    const hooks = buildLeadHooks(store);
  ...
  test("trackStatuses option narrows the observation window", async () => {
    const store = new InMemoryHarnessStateStore();
    await store.createTask({ harnessId: "w1", title: "draft only", goal: "g" });
    ...
    await store.createTask({ harnessId: "w1", title: "open task", goal: "g", status: "open" });
    await store.createTask({ harnessId: "w1", title: "another draft", goal: "g" });
