#!/usr/bin/env bun
/**
 * Harness A2A test harness.
 *
 * Usage:
 *   bun packages/harness/test/a2a/harness-harness.ts [phase1|phase2|phase3|all|T<N>]
 *
...
switch (arg) {
  ...
  default:
    if (arg.startsWith("T")) {
      ...
      console.error(`Usage: harness-harness.ts [phase1|phase2|phase3|all|T<N>]`);
      process.exit(1);
    }
}

console.log(`\n${c.bold}Harness A2A Tests${c.reset} — running ${toRun.length} test(s)\n`);
await run(toRun);
