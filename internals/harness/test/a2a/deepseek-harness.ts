#!/usr/bin/env bun
/**
 * Harness A2A test with DeepSeek backend.
 *
 * Tests multi-agent collaboration using DeepSeek LLM via AI SDK.
 *
 * Usage:
 *   DEEPSEEK_API_KEY=sk-... bun packages/harness/test/a2a/deepseek-harness.ts
 */
...
async function testSingleAgentHarness(): Promise<void> {
  console.log(`\n${c.bold}T-DS2: Single Agent in Harness${c.reset}`);
...
async function testToolUsageViaHarness(): Promise<void> {
  console.log(`\n${c.bold}T-DS4: Harness Tools (Documents + Resources)${c.reset}`);
...
console.log(`${c.bold}Harness A2A Tests — DeepSeek Backend${c.reset}`);
...
const testMap: Record<string, () => Promise<void>> = {
  "T-DS1": testPreflight,
  "T-DS2": testSingleAgentHarness,
  "T-DS3": testTwoAgentCollaboration,
  "T-DS4": testToolUsageViaHarness,
