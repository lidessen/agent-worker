...
Interactive CLI-based tests for the `@agent-worker/harness` package. Tests the full
...
bun -e "import { createWorkspace } from '@agent-worker/harness'; console.log('OK')"
```
...
```ts
import { createWorkspace, MemoryStorage } from "@agent-worker/harness";
...
```ts
import { InstructionQueue, nanoid } from "@agent-worker/harness";
...
```ts
import { createWorkspace, createAgentTools, MemoryStorage } from "@agent-worker/harness";
...
```ts
import { createWorkspace, createWiredLoop, MemoryStorage } from "@agent-worker/harness";
