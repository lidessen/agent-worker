 ...
 * reach back through the workspace state store rather than re-querying
...
export interface CreateWakeToolsOptions {
  /** Workspace state store — the Wake is read/updated here. */
...
export function createWakeTools(
  ...
  const worktreesRoot = join(dataDir, "workspace-data", safeKey, "worktrees", wakeId);
...
/** Tool definition metadata for the Wake-scoped tools — same shape as `WORKSPACE_TOOL_DEFS`. */
export const WAKE_TOOL_DEFS = {
  worktree_create: {
    description:
      "Provision a git worktree for the current Wake. Caller decides `name` (Wake-scoped unique) and `branch` name. Workspace runtime allocates the path. Provisioned worktrees are torn down automatically when the Wake transitions to a terminal status.",
    parameters: {
      name: {
        type: "string",
